...
", header = TRUE)
data <- data[, -ncol(data)]
clara_result <- clara(data, k = 3, samples = 10)
print("Cluster Medoids:")
print(clara_result$medoids)
numeric_data <- data[, sapply(data, is.numeric)]
pca <- prcomp(numeric_data, scale. = TRUE)
if (is.null(pca)) {
  print("Error: PCA failed. Check your data for non-numeric values.")
} else {
  data_pca <- as.data.frame(pca$x[, 1:2])
  colnames(data_pca) <- c("PC1", "PC2")
  data_pca$cluster <- as.factor(clara_result$clustering)
  ggplot(data_pca, aes(x = PC1, y = PC2, color = cluster)) +
    geom_point() +
    ggtitle("CLARA Clustering Result") +
