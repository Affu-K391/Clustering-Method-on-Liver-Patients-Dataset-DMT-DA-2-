...
temp_file <- tempfile()
writeLines(dataset_text, temp_file)

# Read the dataset from the temporary file
data <- read.table(temp_file, header = TRUE)

# Perform k-means clustering
set.seed(123) # for reproducibility
kmeans_result <- kmeans(data[, c(3:10)], centers = 3)

# Add cluster labels to the original dataset
data$cluster <- kmeans_result$cluster

# Print the dataset with cluster labels
print(head(data)) # Print only the first few rows due to space limitation
...
# Plot the clusters based on 'Total_Protiens' and 'Albumin'
ggplot(data_df, aes(x = Total_Protiens, y = Albumin, color = factor(cluster))) +
  geom_point() +
  scale_color_manual(values = c("red", "blue", "green")) +
  labs(title = "Clusters based on Total_Protiens and Albumin", x = "Total_Protiens", y = "Albumin") +
  theme_minimal()
