...
data_df <- read.table(text=data_text, header=TRUE)
# Handle missing values
data_df <- na.omit(data_df)  # Remove rows with missing values, adjust as needed

# Encode categorical variables
data_df$Gender <- as.numeric(factor(data_df$Gender))  # Encode 'Gender' to numeric

# Split Data into Features (X) and Target (y)
X <- data_df[, -ncol(data_df)]  # Features
y <- data_df[, ncol(data_df)]   # Target

# Train-Test Split
set.seed(123)  # For reproducibility
train_indices <- sample(1:nrow(data_df), 0.7 * nrow(data_df))  # 70% for training
...
if (any_na) {
  print("Missing values detected in the dataset. Please handle missing values appropriately.")
} else {
  # KNN Clustering
...
# Plot the graph
plot(X_test[,1], X_test[,2], col = class_colors[y_test], pch = 19, 
     main = "KNN Classification", xlab = "Feature 1", ylab = "Feature 2")

# Add legend
legend("topright", legend = unique(y_test), col = class_colors, pch = 19, 
       title = "Classes", cex = 0.8)

# Add labels for predicted classes
text(X_test[,1], X_test[,2], labels = predicted, pos = 3, col = "black", cex = 0.7)
